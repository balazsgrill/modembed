 ...
 */
public class InstructionsetMinimalModelHelper {
	
	private final static hu.modembed.model.textnotations.instructionset.util.InstructionsetEClassUtil eClassUtil = new hu.modembed.model.textnotations.instructionset.util.InstructionsetEClassUtil();
			...
			if (feature instanceof org.eclipse.emf.ecore.EReference) {
				...
				if (type instanceof org.eclipse.emf.ecore.EClass) {
					...
					for (int i = 0; i < lowerBound; i++) {
						...
						if (reference.isContainment()) {
							...
							String initialValue = "#some" + hu.modembed.model.textnotations.instructionset.util.InstructionsetStringUtil.capitalize(typeClass.getName());
						...
						if (value instanceof java.util.List<?>) {
							java.util.List<org.eclipse.emf.ecore.EObject> list = hu.modembed.model.textnotations.instructionset.util.InstructionsetListUtil.castListUnchecked(value);
							list.add(subModel);
						} else {
				...
				if ("EString".equals(attribute.getEType().getName())) {
					...
					if (attribute.getName().equals("name") && name != null) {
						...
						initialValue = "some" + hu.modembed.model.textnotations.instructionset.util.InstructionsetStringUtil.capitalize(attribute.getName());
					}
					Object value = root.eGet(attribute);
					if (value instanceof java.util.List<?>) {
						java.util.List<String> list = hu.modembed.model.textnotations.instructionset.util.InstructionsetListUtil.castListUnchecked(value);
