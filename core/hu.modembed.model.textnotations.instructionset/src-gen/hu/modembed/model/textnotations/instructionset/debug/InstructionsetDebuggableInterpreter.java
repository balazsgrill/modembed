 ...
 */
public class InstructionsetDebuggableInterpreter<ResultType, ContextType> extends hu.modembed.model.textnotations.instructionset.debug.AbstractInstructionsetDebuggable {
	
	/**
	 * The actual interpreter. Interpretation is delegated to this object.
	 */
	private hu.modembed.model.textnotations.instructionset.util.AbstractInstructionsetInterpreter<ResultType, ContextType> interpreterDelegate;
	
	/**
	 * To check whether we must stop the execution after step over/into/return, we use
	 * a closure
	 */
	private hu.modembed.model.textnotations.instructionset.IInstructionsetCommand<org.eclipse.emf.ecore.EObject> stopCondition;
	...
	public InstructionsetDebuggableInterpreter(hu.modembed.model.textnotations.instructionset.util.AbstractInstructionsetInterpreter<ResultType, ContextType> interpreterDelegate, int eventPort) {
		this.eventPort = eventPort;
		this.interpreterDelegate = interpreterDelegate;
		this.interpreterDelegate.addListener(new hu.modembed.model.textnotations.instructionset.IInstructionsetInterpreterListener() {
	...
	public String[] getStack() {
		...
		for (org.eclipse.emf.ecore.EObject parent : parents) {
			String serializedStackElement = hu.modembed.model.textnotations.instructionset.util.InstructionsetStringUtil.encode(',', new String[] {parent.eClass().getName(), Integer.toString(stackFrameID), parent.eResource().getURI().toString(), Integer.toString(getLine(parent)), Integer.toString(getCharStart(parent)), Integer.toString(getCharEnd(parent))});
	...
	public hu.modembed.model.textnotations.instructionset.util.AbstractInstructionsetInterpreter<ResultType, ContextType> getInterpreterDelegate() {
		return interpreterDelegate;
	}
	
	private int getLine(org.eclipse.emf.ecore.EObject element) {
		int line = -1;
		hu.modembed.model.textnotations.instructionset.IInstructionsetLocationMap locationMap = getLocationMap(element);
		if (locationMap != null) {
	...
	private hu.modembed.model.textnotations.instructionset.IInstructionsetLocationMap getLocationMap(org.eclipse.emf.ecore.EObject element) {
	...
	private void evaluateStep(org.eclipse.emf.ecore.EObject element) {
		...
		hu.modembed.model.textnotations.instructionset.IInstructionsetCommand<org.eclipse.emf.ecore.EObject> stopCheck = stopCondition;
		if (stopCheck != null && stopCheck.execute(element)) {
			stopCondition = null;
			// suspending after step...
			setSuspend(true);
			sendEvent(hu.modembed.model.textnotations.instructionset.debug.EInstructionsetDebugMessageTypes.SUSPENDED, true);
			return;
		}
		waitIfSuspended();
	}
	...
	public void stepOver() {
		...
		final int currentLevel = hu.modembed.model.textnotations.instructionset.util.InstructionsetEObjectUtil.getDepth(current);
		stopCondition = new hu.modembed.model.textnotations.instructionset.IInstructionsetCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
				...
				int depth = hu.modembed.model.textnotations.instructionset.util.InstructionsetEObjectUtil.getDepth(element);
	...
	public void stepReturn() {
		...
		final int parentLevel = hu.modembed.model.textnotations.instructionset.util.InstructionsetEObjectUtil.getDepth(current) - 1;
		stopCondition = new hu.modembed.model.textnotations.instructionset.IInstructionsetCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
