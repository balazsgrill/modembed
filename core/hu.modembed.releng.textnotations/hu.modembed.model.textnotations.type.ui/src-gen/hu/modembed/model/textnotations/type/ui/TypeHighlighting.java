 ...
 */
public class TypeHighlighting implements org.eclipse.jface.viewers.ISelectionProvider, org.eclipse.jface.viewers.ISelectionChangedListener {
	...
	private hu.modembed.model.textnotations.type.ui.ITypeTokenScanner scanner;
	...
	private hu.modembed.model.textnotations.type.ui.TypeEditor editor;
	private org.eclipse.jface.text.source.projection.ProjectionViewer projectionViewer;
	private hu.modembed.model.textnotations.type.ui.TypeOccurrence occurrence;
	 ...
	 */
	private final class UpdateHighlightingListener implements org.eclipse.swt.events.KeyListener, org.eclipse.swt.events.VerifyListener, org.eclipse.swt.events.MouseListener, hu.modembed.model.textnotations.type.ITypeBackgroundParsingListener {
	 ...
	 * @param editor
	 */
	public TypeHighlighting(hu.modembed.model.textnotations.type.ITypeTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceviewer, hu.modembed.model.textnotations.type.ui.TypeColorManager colorManager, hu.modembed.model.textnotations.type.ui.TypeEditor editor) {
		this.display = org.eclipse.swt.widgets.Display.getCurrent();
		sourceviewer.getSelectionProvider();
		preferenceStore = hu.modembed.model.textnotations.type.ui.TypeUIPlugin.getDefault().getPreferenceStore();
		this.editor = editor;
		textWidget = sourceviewer.getTextWidget();
		projectionViewer = sourceviewer;
		scanner = new hu.modembed.model.textnotations.type.ui.TypeUIMetaInformation().createTokenScanner(textResource, colorManager);
		occurrence = new hu.modembed.model.textnotations.type.ui.TypeOccurrence(textResource, sourceviewer, scanner);
		bracketSet = new hu.modembed.model.textnotations.type.ui.TypeBracketSet();
		this.colorManager = colorManager;
		isHighlightBrackets = preferenceStore.getBoolean(hu.modembed.model.textnotations.type.ui.TypePreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX);
		bracketColor = colorManager.getColor(org.eclipse.jface.preference.PreferenceConverter.getColor(preferenceStore, hu.modembed.model.textnotations.type.ui.TypePreferenceConstants.EDITOR_MATCHING_BRACKETS_COLOR));
		black = colorManager.getColor(new org.eclipse.swt.graphics.RGB(0, 0, 0));
		
		addListeners(editor);
	}
	
	private void addListeners(hu.modembed.model.textnotations.type.ui.TypeEditor editor) {
	...
	private void setBracketHighlighting(org.eclipse.jface.text.IDocument document) {
		...
		org.eclipse.jface.text.Position[] positions = positionHelper.getPositions(document, hu.modembed.model.textnotations.type.ui.TypePositionCategory.BRACKET.toString());
	...
	private void removeHighlighting() {
		...
		removeHighlightingCategory(document, hu.modembed.model.textnotations.type.ui.TypePositionCategory.BRACKET.toString());
	}
	...
	private void removeHighlightingCategory(org.eclipse.jface.text.IDocument document, String category) {
		...
		if (category.equals(hu.modembed.model.textnotations.type.ui.TypePositionCategory.BRACKET.toString())) {
		...
		display.asyncExec(new Runnable() {
			public void run() {
				...
				if (selectedEObject != null) {
					setSelection(new hu.modembed.model.textnotations.type.ui.TypeEObjectSelection(selectedEObject, false));
				}
			}
		});
	}
