 ...
 * </ul>
 */
public class TypeBracketPreferencePage extends org.eclipse.jface.preference.PreferencePage implements org.eclipse.ui.IWorkbenchPreferencePage {
	
	private static final String[] ALL_LEFT_BRACKETS = new String[] { "{", "(", "[", "<", "\"", "'", };
	private static final String[] ALL_RIGHT_BRACKETS = new String[] { "}", ")", "]", ">", "\"", "'", };
	
	private String BRACKETS_COLOR = hu.modembed.model.textnotations.type.ui.TypePreferenceConstants.EDITOR_MATCHING_BRACKETS_COLOR;
	...
	private String language = new hu.modembed.model.textnotations.type.mopp.TypeMetaInformation().getSyntaxName();
	
	private hu.modembed.model.textnotations.type.ui.TypeBracketSet bracketsTmp;
	
	/**
	 * Creates a preference page for bracket setting.
	 */
	public TypeBracketPreferencePage() {
		super();
		
		hu.modembed.model.textnotations.type.ITypeMetaInformation metaInformation = new hu.modembed.model.textnotations.type.mopp.TypeMetaInformation();
	...
	public void init(org.eclipse.ui.IWorkbench workbench) {
		setPreferenceStore(hu.modembed.model.textnotations.type.ui.TypeUIPlugin.getDefault().getPreferenceStore());
		setDescription("Define the coloring of matching brackets.");
		
		bracketsTmp = new hu.modembed.model.textnotations.type.ui.TypeBracketSet();
		for (String languageID : languageIDs) {
			bracketSetTemp.put(languageID, getPreferenceStore().getString(languageID + hu.modembed.model.textnotations.type.ui.TypePreferenceConstants.EDITOR_BRACKETS_SUFFIX));
		}
	}
	...
	private void handleMatchingBracketsSelection() {
		...
		enableCheckbox.setSelection(getPreferenceStore().getBoolean(hu.modembed.model.textnotations.type.ui.TypePreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		enableClosingInside.setSelection(false);
		matchingBracketsColorButton.setEnabled(getPreferenceStore().getBoolean(		hu.modembed.model.textnotations.type.ui.TypePreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		...
		initializeLanguage();
		bracketsTmp.deserialize(getPreferenceStore().getString(language + hu.modembed.model.textnotations.type.ui.TypePreferenceConstants.EDITOR_BRACKETS_SUFFIX));
	...
	private void addListenersToStyleButtons() {
		...
		bracketsList.addSelectionListener(new org.eclipse.swt.events.SelectionListener() {
			...
			public void widgetSelected(org.eclipse.swt.events.SelectionEvent e) {
				hu.modembed.model.textnotations.type.ITypeBracketPair bracketPair = getSelectedBracketPair();
				if (bracketPair == null) {
					removeBracketButton.setEnabled(false);
					return;
				}
	...
	protected void performDefaults() {
		...
		enableCheckbox.setSelection(preferenceStore.getDefaultBoolean(hu.modembed.model.textnotations.type.ui.TypePreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		matchingBracketsColorButton.setEnabled(enableCheckbox.getSelection());
		...
		String defaultBrackets = preferenceStore.getDefaultString(language + hu.modembed.model.textnotations.type.ui.TypePreferenceConstants.EDITOR_BRACKETS_SUFFIX);
	...
	private void updateActiveEditor() {
		...
		preferenceStore.setValue(hu.modembed.model.textnotations.type.ui.TypePreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX, enableCheckbox.getSelection());
		preferenceStore.setValue(language + hu.modembed.model.textnotations.type.ui.TypePreferenceConstants.EDITOR_BRACKETS_SUFFIX, bracketSetTemp.get(language));
		...
		if (editor != null && editor instanceof hu.modembed.model.textnotations.type.ui.TypeEditor) {
			((hu.modembed.model.textnotations.type.ui.TypeEditor) editor).invalidateTextRepresentation();
		}
	}
	
	protected hu.modembed.model.textnotations.type.ITypeBracketPair getSelectedBracketPair() {
