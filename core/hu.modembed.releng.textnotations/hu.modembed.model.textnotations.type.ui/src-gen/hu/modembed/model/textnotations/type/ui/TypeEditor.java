...
/**
 * A text editor for 'type' models.
 * <p>
 * This editor has id
 * <code>hu.modembed.model.textnotations.type.ui.TypeEditor</code>
 * The editor's context menu has id
 * <code>hu.modembed.model.textnotations.type.EditorContext</code>.
 * The editor's ruler context menu has id
 * <code>hu.modembed.model.textnotations.type.EditorRuler</code>.
 * </p>
 */
public class TypeEditor extends org.eclipse.ui.editors.text.TextEditor implements org.eclipse.emf.edit.domain.IEditingDomainProvider, org.eclipse.jface.viewers.ISelectionProvider, org.eclipse.jface.viewers.ISelectionChangedListener, org.eclipse.emf.common.ui.viewer.IViewerProvider, hu.modembed.model.textnotations.type.ITypeResourceProvider, hu.modembed.model.textnotations.type.ui.ITypeBracketHandlerProvider, hu.modembed.model.textnotations.type.ui.ITypeAnnotationModelProvider {
	
	private hu.modembed.model.textnotations.type.ui.TypeHighlighting highlighting;
	private org.eclipse.jface.text.source.projection.ProjectionSupport projectionSupport;
	private hu.modembed.model.textnotations.type.ui.TypeCodeFoldingManager codeFoldingManager;
	private hu.modembed.model.textnotations.type.ui.TypeBackgroundParsingStrategy bgParsingStrategy = new hu.modembed.model.textnotations.type.ui.TypeBackgroundParsingStrategy();
	private java.util.Collection<hu.modembed.model.textnotations.type.ITypeBackgroundParsingListener> bgParsingListeners = new java.util.ArrayList<hu.modembed.model.textnotations.type.ITypeBackgroundParsingListener>();
	private hu.modembed.model.textnotations.type.ui.TypeColorManager colorManager = new hu.modembed.model.textnotations.type.ui.TypeColorManager();
	private hu.modembed.model.textnotations.type.ui.TypeOutlinePage outlinePage;
	...
	private hu.modembed.model.textnotations.type.ui.TypePropertySheetPage propertySheetPage;
	...
	private hu.modembed.model.textnotations.type.ui.ITypeBracketHandler bracketHandler;
	...
	public TypeEditor() {
		super();
		setSourceViewerConfiguration(new hu.modembed.model.textnotations.type.ui.TypeSourceViewerConfiguration(this, this, colorManager));
	...
	private final class DocumentListener implements org.eclipse.jface.text.IDocumentListener {
		...
		public void documentChanged(org.eclipse.jface.text.DocumentEvent event) {
			bgParsingStrategy.parse(event, getResource(), TypeEditor.this);
		}
	}
	...
	private class ModelResourceChangeListener implements org.eclipse.core.resources.IResourceChangeListener {
		public void resourceChanged(org.eclipse.core.resources.IResourceChangeEvent event) {
			...
			try {
					...
					public boolean visit(org.eclipse.core.resources.IResourceDelta delta) {
						...
						if (deltaKind == org.eclipse.core.resources.IResourceDelta.CHANGED && delta.getFlags() != org.eclipse.core.resources.IResourceDelta.MARKERS) {
							...
							if (changedResource != null) {
								...
								hu.modembed.model.textnotations.type.ITypeTextResource currentResource = getResource();
				...
				hu.modembed.model.textnotations.type.ui.TypeUIPlugin.logError("Unexpected Error: ", exception);
			}
		}
	}
	...
	public void initializeEditor() {
		...
		setEditorContextMenuId("hu.modembed.model.textnotations.type.EditorContext");
		setRulerContextMenuId("hu.modembed.model.textnotations.type.EditorRuler");
	}
	...
	public void createPartControl(org.eclipse.swt.widgets.Composite parent) {
		...
		highlighting = new hu.modembed.model.textnotations.type.ui.TypeHighlighting(getResource(), viewer, colorManager, this);
		...
		codeFoldingManager = new hu.modembed.model.textnotations.type.ui.TypeCodeFoldingManager(viewer, this);
	}
	...
	private void initializeResourceObject(org.eclipse.ui.IEditorInput editorInput) {
		...
		hu.modembed.model.textnotations.type.mopp.TypeNature.activate(inputFile.getProject());
		...
		hu.modembed.model.textnotations.type.ITypeTextResource loadedResource = (hu.modembed.model.textnotations.type.ITypeTextResource) resourceSet.getResource(uri, false);
		if (loadedResource == null) {
			try {
				...
				hu.modembed.model.textnotations.type.ITypeTextResource currentResource = this.resource;
				...
				if (demandLoadedResource instanceof hu.modembed.model.textnotations.type.ITypeTextResource) {
					setResource((hu.modembed.model.textnotations.type.ITypeTextResource) demandLoadedResource);
				} else {
					// the resource was not loaded by an EMFText resource, but some other EMF resource
					hu.modembed.model.textnotations.type.ui.TypeUIPlugin.showErrorDialog("No EMFText resource.", "The file '" + uri.lastSegment() + "' of type '" + uri.fileExtension() + "' can not be handled by the TypeEditor.");
					// close this editor because it can not present the resource
					close(false);
				}
			} catch (Exception e) {
				hu.modembed.model.textnotations.type.ui.TypeUIPlugin.logError("Exception while loading resource in " + this.getClass().getSimpleName() + ".", e);
			}
		} else {
	...
	private void setResource(hu.modembed.model.textnotations.type.ITypeTextResource resource) {
	...
	private Object getOutlinePage() {
		if (outlinePage == null) {
			outlinePage = new hu.modembed.model.textnotations.type.ui.TypeOutlinePage(this);
	...
	public org.eclipse.ui.views.properties.IPropertySheetPage getPropertySheetPage() {
		if (propertySheetPage == null) {
			propertySheetPage = new hu.modembed.model.textnotations.type.ui.TypePropertySheetPage();
	...
	public void setCaret(org.eclipse.emf.ecore.EObject element, String text) {
		try {
			...
			org.eclipse.jface.text.source.ISourceViewer viewer = getSourceViewer();
			hu.modembed.model.textnotations.type.ITypeTextResource textResource = (hu.modembed.model.textnotations.type.ITypeTextResource) element.eResource();
			...
			hu.modembed.model.textnotations.type.ITypeTextScanner lexer = getResource().getMetaInformation().createLexer();
			try {
				lexer.setText(viewer.getDocument().get(destination, length));
				hu.modembed.model.textnotations.type.ITypeTextToken token = lexer.getNextToken();
			...
			hu.modembed.model.textnotations.type.ui.TypeUIPlugin.logError("Exception in setCaret()", e);
		}
	}
	...
	protected org.eclipse.jface.text.source.ISourceViewer createSourceViewer(org.eclipse.swt.widgets.Composite parent, org.eclipse.jface.text.source.IVerticalRuler ruler, int styles) {
		org.eclipse.jface.text.source.ISourceViewer viewer = new org.eclipse.jface.text.source.projection.ProjectionViewer(parent, ruler, getOverviewRuler(), isOverviewRulerVisible(), styles) {
			...
			public void setSelection(org.eclipse.jface.viewers.ISelection selection, boolean reveal) {
				if (!TypeEditor.this.setSelection(selection, reveal)) {
	...
	public void addBackgroundParsingListener(hu.modembed.model.textnotations.type.ITypeBackgroundParsingListener listener) {
		bgParsingListeners.add(listener);
	}
	
	public void notifyBackgroundParsingFinished() {
		for (hu.modembed.model.textnotations.type.ITypeBackgroundParsingListener listener : bgParsingListeners) {
			listener.parsingCompleted(getResource());
		}
	}
	
	public hu.modembed.model.textnotations.type.ui.ITypeBracketHandler getBracketHandler() {
		return bracketHandler;
	}
	
	public void setBracketHandler(hu.modembed.model.textnotations.type.ui.ITypeBracketHandler bracketHandler) {
	...
	private boolean setSelection(org.eclipse.jface.viewers.ISelection selection, boolean reveal) {
		if (selection instanceof org.eclipse.jface.viewers.IStructuredSelection) {
			...
			if (object instanceof org.eclipse.emf.ecore.EObject) {
				...
				if (!(resource instanceof hu.modembed.model.textnotations.type.ITypeTextResource)) {
					return false;
				}
				hu.modembed.model.textnotations.type.ITypeTextResource textResource = (hu.modembed.model.textnotations.type.ITypeTextResource) resource;
				hu.modembed.model.textnotations.type.ITypeLocationMap locationMap = textResource.getLocationMap();
				int destination = locationMap.getCharStart(element);
				if (destination < 0) {
					destination = 0;
