...
public class InstructionsetDefaultNameProvider implements hu.modembed.model.textnotations.instructionset.IInstructionsetNameProvider {
		...
		if (nameAttr instanceof org.eclipse.emf.ecore.EAttribute) {
			...
			for (org.eclipse.emf.ecore.EOperation operation : element.eClass().getEAllOperations()) {
				if (operation.getName().toLowerCase().endsWith(NAME_FEATURE) && operation.getEParameters().size() == 0) {
					Object result = hu.modembed.model.textnotations.instructionset.util.InstructionsetEObjectUtil.invokeOperation(element, operation);
					if (result != null) {
