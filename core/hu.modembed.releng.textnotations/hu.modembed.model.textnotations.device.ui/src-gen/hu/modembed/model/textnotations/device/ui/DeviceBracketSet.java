...
public class DeviceBracketSet {
	...
	private java.util.ArrayList<hu.modembed.model.textnotations.device.IDeviceBracketPair> bracketPairs;
	private String languageID;
	
	/**
	 * Creates a new bracket set to manage bracket pairs.
	 */
	public DeviceBracketSet() {
		super();
		this.languageID = new hu.modembed.model.textnotations.device.mopp.DeviceMetaInformation().getSyntaxName();
		this.bracketPairs = new java.util.ArrayList<hu.modembed.model.textnotations.device.IDeviceBracketPair>();
	}
	...
	public hu.modembed.model.textnotations.device.IDeviceBracketPair getBracketPair(int index) {
		try {
	 ...
	 */
	public hu.modembed.model.textnotations.device.IDeviceBracketPair getBracketPair(String opening, String closing) {
		for (hu.modembed.model.textnotations.device.IDeviceBracketPair bracketPair : bracketPairs) {
	...
	public boolean addBracketPair(String opening, String closing, boolean closingEnabledInside, boolean closeAfterEnter) {
		...
		bracketPairs.add(new hu.modembed.model.textnotations.device.mopp.DeviceBracketPair(opening, closing, closingEnabledInside, closeAfterEnter));
		return true;
	}
	...
	public boolean resetBrackets(org.eclipse.jface.preference.IPreferenceStore preferenceStore) {
		String bracketPairs = preferenceStore.getString(languageID + hu.modembed.model.textnotations.device.ui.DevicePreferenceConstants.EDITOR_BRACKETS_SUFFIX);
	 ...
	 */
	public hu.modembed.model.textnotations.device.IDeviceBracketPair remove(String opening, String closing) {
		for (hu.modembed.model.textnotations.device.IDeviceBracketPair bracketPair : bracketPairs) {
			if (bracketPair.getOpeningBracket().equals(opening) && bracketPair.getClosingBracket().equals(closing)) {
	...
	public void deserialize(String bracketSet) {
		bracketPairs = new java.util.ArrayList<hu.modembed.model.textnotations.device.IDeviceBracketPair>();
	...
	public void findAndHighlightMatchingBrackets(org.eclipse.jface.text.IDocument document, int caretOffset) {
		...
		if (position != -1 && position != document.getLength()) {
			positionHelper.addPosition(document, hu.modembed.model.textnotations.device.ui.DevicePositionCategory.BRACKET.toString(), position, 1);
			positionHelper.addPosition(document, hu.modembed.model.textnotations.device.ui.DevicePositionCategory.BRACKET.toString(), caretOffset - 1, 1);
		}
	}
