...
/**
 * A text editor for 'device' models.
 * <p>
 * This editor has id
 * <code>hu.modembed.model.textnotations.device.ui.DeviceEditor</code>
 * The editor's context menu has id
 * <code>hu.modembed.model.textnotations.device.EditorContext</code>.
 * The editor's ruler context menu has id
 * <code>hu.modembed.model.textnotations.device.EditorRuler</code>.
 * </p>
 */
public class DeviceEditor extends org.eclipse.ui.editors.text.TextEditor implements org.eclipse.emf.edit.domain.IEditingDomainProvider, org.eclipse.jface.viewers.ISelectionProvider, org.eclipse.jface.viewers.ISelectionChangedListener, org.eclipse.emf.common.ui.viewer.IViewerProvider, hu.modembed.model.textnotations.device.IDeviceResourceProvider, hu.modembed.model.textnotations.device.ui.IDeviceBracketHandlerProvider, hu.modembed.model.textnotations.device.ui.IDeviceAnnotationModelProvider {
	
	private hu.modembed.model.textnotations.device.ui.DeviceHighlighting highlighting;
	private org.eclipse.jface.text.source.projection.ProjectionSupport projectionSupport;
	private hu.modembed.model.textnotations.device.ui.DeviceCodeFoldingManager codeFoldingManager;
	private hu.modembed.model.textnotations.device.ui.DeviceBackgroundParsingStrategy bgParsingStrategy = new hu.modembed.model.textnotations.device.ui.DeviceBackgroundParsingStrategy();
	private java.util.Collection<hu.modembed.model.textnotations.device.IDeviceBackgroundParsingListener> bgParsingListeners = new java.util.ArrayList<hu.modembed.model.textnotations.device.IDeviceBackgroundParsingListener>();
	private hu.modembed.model.textnotations.device.ui.DeviceColorManager colorManager = new hu.modembed.model.textnotations.device.ui.DeviceColorManager();
	private hu.modembed.model.textnotations.device.ui.DeviceOutlinePage outlinePage;
	...
	private hu.modembed.model.textnotations.device.ui.DevicePropertySheetPage propertySheetPage;
	...
	private hu.modembed.model.textnotations.device.ui.IDeviceBracketHandler bracketHandler;
	...
	public DeviceEditor() {
		super();
		setSourceViewerConfiguration(new hu.modembed.model.textnotations.device.ui.DeviceSourceViewerConfiguration(this, this, colorManager));
	...
	private final class DocumentListener implements org.eclipse.jface.text.IDocumentListener {
		...
		public void documentChanged(org.eclipse.jface.text.DocumentEvent event) {
			bgParsingStrategy.parse(event, getResource(), DeviceEditor.this);
		}
	}
	...
	private class ModelResourceChangeListener implements org.eclipse.core.resources.IResourceChangeListener {
		public void resourceChanged(org.eclipse.core.resources.IResourceChangeEvent event) {
			...
			try {
					...
					public boolean visit(org.eclipse.core.resources.IResourceDelta delta) {
						...
						if (deltaKind == org.eclipse.core.resources.IResourceDelta.CHANGED && delta.getFlags() != org.eclipse.core.resources.IResourceDelta.MARKERS) {
							...
							if (changedResource != null) {
								...
								hu.modembed.model.textnotations.device.IDeviceTextResource currentResource = getResource();
				...
				hu.modembed.model.textnotations.device.ui.DeviceUIPlugin.logError("Unexpected Error: ", exception);
			}
		}
	}
	...
	public void initializeEditor() {
		...
		setEditorContextMenuId("hu.modembed.model.textnotations.device.EditorContext");
		setRulerContextMenuId("hu.modembed.model.textnotations.device.EditorRuler");
	}
	...
	public void createPartControl(org.eclipse.swt.widgets.Composite parent) {
		...
		highlighting = new hu.modembed.model.textnotations.device.ui.DeviceHighlighting(getResource(), viewer, colorManager, this);
		...
		codeFoldingManager = new hu.modembed.model.textnotations.device.ui.DeviceCodeFoldingManager(viewer, this);
	}
	...
	private void initializeResourceObject(org.eclipse.ui.IEditorInput editorInput) {
		...
		hu.modembed.model.textnotations.device.mopp.DeviceNature.activate(inputFile.getProject());
		...
		hu.modembed.model.textnotations.device.IDeviceTextResource loadedResource = (hu.modembed.model.textnotations.device.IDeviceTextResource) resourceSet.getResource(uri, false);
		if (loadedResource == null) {
			try {
				...
				hu.modembed.model.textnotations.device.IDeviceTextResource currentResource = this.resource;
				...
				if (demandLoadedResource instanceof hu.modembed.model.textnotations.device.IDeviceTextResource) {
					setResource((hu.modembed.model.textnotations.device.IDeviceTextResource) demandLoadedResource);
				} else {
					// the resource was not loaded by an EMFText resource, but some other EMF resource
					hu.modembed.model.textnotations.device.ui.DeviceUIPlugin.showErrorDialog("No EMFText resource.", "The file '" + uri.lastSegment() + "' of type '" + uri.fileExtension() + "' can not be handled by the DeviceEditor.");
					// close this editor because it can not present the resource
					close(false);
				}
			} catch (Exception e) {
				hu.modembed.model.textnotations.device.ui.DeviceUIPlugin.logError("Exception while loading resource in " + this.getClass().getSimpleName() + ".", e);
			}
		} else {
	...
	private void setResource(hu.modembed.model.textnotations.device.IDeviceTextResource resource) {
	...
	private Object getOutlinePage() {
		if (outlinePage == null) {
			outlinePage = new hu.modembed.model.textnotations.device.ui.DeviceOutlinePage(this);
	...
	public org.eclipse.ui.views.properties.IPropertySheetPage getPropertySheetPage() {
		if (propertySheetPage == null) {
			propertySheetPage = new hu.modembed.model.textnotations.device.ui.DevicePropertySheetPage();
	...
	public void setCaret(org.eclipse.emf.ecore.EObject element, String text) {
		try {
			...
			org.eclipse.jface.text.source.ISourceViewer viewer = getSourceViewer();
			hu.modembed.model.textnotations.device.IDeviceTextResource textResource = (hu.modembed.model.textnotations.device.IDeviceTextResource) element.eResource();
			hu.modembed.model.textnotations.device.IDeviceLocationMap locationMap = textResource.getLocationMap();
			int destination = locationMap.getCharStart(element);
			int length = locationMap.getCharEnd(element) + 1 - destination;
			
			hu.modembed.model.textnotations.device.IDeviceTextScanner lexer = getResource().getMetaInformation().createLexer();
			try {
				lexer.setText(viewer.getDocument().get(destination, length));
				hu.modembed.model.textnotations.device.IDeviceTextToken token = lexer.getNextToken();
			...
			hu.modembed.model.textnotations.device.ui.DeviceUIPlugin.logError("Exception in setCaret()", e);
		}
	}
	...
	protected org.eclipse.jface.text.source.ISourceViewer createSourceViewer(org.eclipse.swt.widgets.Composite parent, org.eclipse.jface.text.source.IVerticalRuler ruler, int styles) {
		org.eclipse.jface.text.source.ISourceViewer viewer = new org.eclipse.jface.text.source.projection.ProjectionViewer(parent, ruler, getOverviewRuler(), isOverviewRulerVisible(), styles) {
			...
			public void setSelection(org.eclipse.jface.viewers.ISelection selection, boolean reveal) {
				if (!DeviceEditor.this.setSelection(selection, reveal)) {
	...
	public void addBackgroundParsingListener(hu.modembed.model.textnotations.device.IDeviceBackgroundParsingListener listener) {
		bgParsingListeners.add(listener);
	}
	
	public void notifyBackgroundParsingFinished() {
		for (hu.modembed.model.textnotations.device.IDeviceBackgroundParsingListener listener : bgParsingListeners) {
			listener.parsingCompleted(getResource());
		}
	}
	
	public hu.modembed.model.textnotations.device.ui.IDeviceBracketHandler getBracketHandler() {
		return bracketHandler;
	}
	
	public void setBracketHandler(hu.modembed.model.textnotations.device.ui.IDeviceBracketHandler bracketHandler) {
	...
	private boolean setSelection(org.eclipse.jface.viewers.ISelection selection, boolean reveal) {
		if (selection instanceof org.eclipse.jface.viewers.IStructuredSelection) {
			...
			if (object instanceof org.eclipse.emf.ecore.EObject) {
				...
				if (!(resource instanceof hu.modembed.model.textnotations.device.IDeviceTextResource)) {
					return false;
				}
				hu.modembed.model.textnotations.device.IDeviceTextResource textResource = (hu.modembed.model.textnotations.device.IDeviceTextResource) resource;
				hu.modembed.model.textnotations.device.IDeviceLocationMap locationMap = textResource.getLocationMap();
				int destination = locationMap.getCharStart(element);
				if (destination < 0) {
					destination = 0;
