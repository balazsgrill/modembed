 ...
 */
public class DeviceHighlighting implements org.eclipse.jface.viewers.ISelectionProvider, org.eclipse.jface.viewers.ISelectionChangedListener {
	
	private final static hu.modembed.model.textnotations.device.ui.DevicePositionHelper positionHelper = new hu.modembed.model.textnotations.device.ui.DevicePositionHelper();
	...
	private hu.modembed.model.textnotations.device.ui.IDeviceTokenScanner scanner;
	...
	private hu.modembed.model.textnotations.device.ui.DeviceOccurrence occurrence;
	private hu.modembed.model.textnotations.device.ui.DeviceBracketSet bracketSet;
	 ...
	 */
	private final class UpdateHighlightingListener implements org.eclipse.swt.events.KeyListener, org.eclipse.swt.events.VerifyListener, org.eclipse.swt.events.MouseListener, hu.modembed.model.textnotations.device.IDeviceBackgroundParsingListener {
	 ...
	 * @param editor
	 */
	public DeviceHighlighting(hu.modembed.model.textnotations.device.IDeviceTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceviewer, hu.modembed.model.textnotations.device.ui.DeviceColorManager colorManager, hu.modembed.model.textnotations.device.ui.DeviceEditor editor) {
		this.display = org.eclipse.swt.widgets.Display.getCurrent();
		sourceviewer.getSelectionProvider();
		preferenceStore = hu.modembed.model.textnotations.device.ui.DeviceUIPlugin.getDefault().getPreferenceStore();
		this.editor = editor;
		textWidget = sourceviewer.getTextWidget();
		projectionViewer = sourceviewer;
		scanner = new hu.modembed.model.textnotations.device.ui.DeviceUIMetaInformation().createTokenScanner(textResource, colorManager);
		occurrence = new hu.modembed.model.textnotations.device.ui.DeviceOccurrence(textResource, sourceviewer, scanner);
		bracketSet = new hu.modembed.model.textnotations.device.ui.DeviceBracketSet();
		this.colorManager = colorManager;
		isHighlightBrackets = preferenceStore.getBoolean(hu.modembed.model.textnotations.device.ui.DevicePreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX);
		bracketColor = colorManager.getColor(org.eclipse.jface.preference.PreferenceConverter.getColor(preferenceStore, hu.modembed.model.textnotations.device.ui.DevicePreferenceConstants.EDITOR_MATCHING_BRACKETS_COLOR));
		black = colorManager.getColor(new org.eclipse.swt.graphics.RGB(0, 0, 0));
		
		addListeners(editor);
	}
	
	private void addListeners(hu.modembed.model.textnotations.device.ui.DeviceEditor editor) {
	...
	private void setBracketHighlighting(org.eclipse.jface.text.IDocument document) {
		...
		org.eclipse.jface.text.Position[] positions = positionHelper.getPositions(document, hu.modembed.model.textnotations.device.ui.DevicePositionCategory.BRACKET.toString());
	...
	private void removeHighlighting() {
		...
		removeHighlightingCategory(document, hu.modembed.model.textnotations.device.ui.DevicePositionCategory.BRACKET.toString());
	}
	...
	private void removeHighlightingCategory(org.eclipse.jface.text.IDocument document, String category) {
		...
		if (category.equals(hu.modembed.model.textnotations.device.ui.DevicePositionCategory.BRACKET.toString())) {
		...
		display.asyncExec(new Runnable() {
			public void run() {
				...
				if (selectedEObject != null) {
					setSelection(new hu.modembed.model.textnotations.device.ui.DeviceEObjectSelection(selectedEObject, false));
				}
			}
		});
	}
