 ...
 */
public class DeviceOccurrence {
	
	public final static String OCCURRENCE_ANNOTATION_ID = "hu.modembed.model.textnotations.device.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "hu.modembed.model.textnotations.device.ui.occurences.declaration";
	...
	private hu.modembed.model.textnotations.device.ui.IDeviceTokenScanner tokenScanner;
	...
	private hu.modembed.model.textnotations.device.IDeviceTextResource textResource;
	 ...
	 */
	public DeviceOccurrence(hu.modembed.model.textnotations.device.IDeviceTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, hu.modembed.model.textnotations.device.ui.IDeviceTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new hu.modembed.model.textnotations.device.mopp.DeviceMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(hu.modembed.model.textnotations.device.ui.DeviceBracketSet bracketSet) {
		if (textResource == null) {
			return;
		}
		org.eclipse.swt.custom.StyledText textWidget = projectionViewer.getTextWidget();
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addAnnotation(document, hu.modembed.model.textnotations.device.ui.DevicePositionCategory.DEFINTION, text);
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if (elementsAtDefinition.contains(occEO) || definitionElement.equals(occEO)) {
						addAnnotation(document, hu.modembed.model.textnotations.device.ui.DevicePositionCategory.PROXY, text);
					}
				}
			}
			token = tokenScanner.nextToken();
	...
	private void addAnnotation(org.eclipse.jface.text.IDocument document, hu.modembed.model.textnotations.device.ui.DevicePositionCategory type, String text) {
		int tokenOffset = tokenScanner.getTokenOffset();
		int tokenLength = tokenScanner.getTokenLength();
		...
		if (type == hu.modembed.model.textnotations.device.ui.DevicePositionCategory.DEFINTION) {
	...
	private void removeAnnotations() {
		removeAnnotations(hu.modembed.model.textnotations.device.ui.DeviceOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(hu.modembed.model.textnotations.device.ui.DeviceOccurrence.DECLARATION_ANNOTATION_ID);
	}
