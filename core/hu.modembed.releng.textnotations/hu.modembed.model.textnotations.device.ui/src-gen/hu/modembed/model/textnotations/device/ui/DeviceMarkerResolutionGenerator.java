...
public class DeviceMarkerResolutionGenerator implements org.eclipse.ui.IMarkerResolutionGenerator {
	...
	public org.eclipse.ui.IMarkerResolution[] getResolutions(org.eclipse.core.resources.IMarker marker) {
		try {
			...
			if (resource instanceof org.eclipse.core.resources.IFile) {
				...
				rs.getLoadOptions().put(hu.modembed.model.textnotations.device.IDeviceOptions.DISABLE_CREATING_MARKERS_FOR_PROBLEMS, "true");
				org.eclipse.emf.ecore.resource.Resource emfResource = rs.getResource(uri, true);
				if (emfResource instanceof hu.modembed.model.textnotations.device.mopp.DeviceResource) {
					hu.modembed.model.textnotations.device.mopp.DeviceResource customResource = (hu.modembed.model.textnotations.device.mopp.DeviceResource) emfResource;
					org.eclipse.emf.ecore.util.EcoreUtil.resolveAll(customResource);
					java.util.Collection<hu.modembed.model.textnotations.device.IDeviceQuickFix> quickFixes = getQuickFixes(customResource, marker);
					java.util.List<org.eclipse.ui.IMarkerResolution2> resolutions = new java.util.ArrayList<org.eclipse.ui.IMarkerResolution2>();
					for (final hu.modembed.model.textnotations.device.IDeviceQuickFix quickFix : quickFixes) {
						resolutions.add(new org.eclipse.ui.IMarkerResolution2() {
							...
							public void run(org.eclipse.core.resources.IMarker marker) {
								...
								try {
									...
									hu.modembed.model.textnotations.device.ui.DeviceUIPlugin.logError("Exception while applying quick fix", e);
								}
							}
			...
			hu.modembed.model.textnotations.device.ui.DeviceUIPlugin.logError("Exception while computing quick fix resolutions", e);
		}
		return new org.eclipse.ui.IMarkerResolution[] {};
	}
	
	public java.util.Collection<hu.modembed.model.textnotations.device.IDeviceQuickFix> getQuickFixes(hu.modembed.model.textnotations.device.IDeviceTextResource resource, org.eclipse.core.resources.IMarker marker) {
		...
		try {
			...
			if (quickFixContexts != null) {
				...
				for (String quickFixContext : quickFixContextParts) {
					hu.modembed.model.textnotations.device.IDeviceQuickFix quickFix = resource.getQuickFix(quickFixContext);
					if (quickFix != null) {
