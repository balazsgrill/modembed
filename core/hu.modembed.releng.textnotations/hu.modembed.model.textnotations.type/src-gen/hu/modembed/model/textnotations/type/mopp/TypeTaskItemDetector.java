...
/**
 * The TypeTaskItemDetector is used to find task items in text documents. The
 * current implementation searches for specific keywords to detect task items. The
 * TypeTaskItemDetector is used both by the TaskItemBuilder and the editor.
 */
public class TypeTaskItemDetector {
	
	public static String[] TASK_ITEM_KEYWORDS = new String[] {"TODO", "FIXME", "XXX"};
	
	public java.util.List<hu.modembed.model.textnotations.type.mopp.TypeTaskItem> findTaskItems(String text, int line, int charStart) {
		java.util.List<hu.modembed.model.textnotations.type.mopp.TypeTaskItem> foundItems = new java.util.ArrayList<hu.modembed.model.textnotations.type.mopp.TypeTaskItem>();
		...
		while (remainingText != null && continueSearch) {
			...
			for (String keyword : TASK_ITEM_KEYWORDS) {
				...
				if (index >= 0) {
					...
					foundItems.add(new hu.modembed.model.textnotations.type.mopp.TypeTaskItem(keyword, message, localLine, offset, end));
