...
public class DeviceResourceFactoryDelegator implements org.eclipse.emf.ecore.resource.Resource.Factory {
	
	protected java.util.Map<String, org.eclipse.emf.ecore.resource.Resource.Factory> factories = null;
	
	public DeviceResourceFactoryDelegator() {
		init();
	}
	...
	protected void init() {
		...
		if (new hu.modembed.model.textnotations.device.util.DeviceRuntimeUtil().isEclipsePlatformAvailable()) {
			new hu.modembed.model.textnotations.device.util.DeviceEclipseProxy().getResourceFactoryExtensions(factories);
		}
		if (factories.get("") == null) {
			factories.put("", new hu.modembed.model.textnotations.device.mopp.DeviceResourceFactory());
		}
	}
