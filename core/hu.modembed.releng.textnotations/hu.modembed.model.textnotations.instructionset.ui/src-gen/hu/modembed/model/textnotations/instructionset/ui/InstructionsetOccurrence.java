 ...
 */
public class InstructionsetOccurrence {
	
	public final static String OCCURRENCE_ANNOTATION_ID = "hu.modembed.model.textnotations.instructionset.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "hu.modembed.model.textnotations.instructionset.ui.occurences.declaration";
	...
	private hu.modembed.model.textnotations.instructionset.ui.IInstructionsetTokenScanner tokenScanner;
	 ...
	 */
	public InstructionsetOccurrence(hu.modembed.model.textnotations.instructionset.IInstructionsetTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, hu.modembed.model.textnotations.instructionset.ui.IInstructionsetTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new hu.modembed.model.textnotations.instructionset.mopp.InstructionsetMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(hu.modembed.model.textnotations.instructionset.ui.InstructionsetBracketSet bracketSet) {
		if (textResource == null) {
			return;
		}
		org.eclipse.swt.custom.StyledText textWidget = projectionViewer.getTextWidget();
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addAnnotation(document, hu.modembed.model.textnotations.instructionset.ui.InstructionsetPositionCategory.DEFINTION, text);
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if (elementsAtDefinition.contains(occEO) || definitionElement.equals(occEO)) {
						addAnnotation(document, hu.modembed.model.textnotations.instructionset.ui.InstructionsetPositionCategory.PROXY, text);
					}
				}
			}
			token = tokenScanner.nextToken();
	...
	private void addAnnotation(org.eclipse.jface.text.IDocument document, hu.modembed.model.textnotations.instructionset.ui.InstructionsetPositionCategory type, String text) {
		int tokenOffset = tokenScanner.getTokenOffset();
		int tokenLength = tokenScanner.getTokenLength();
		...
		if (type == hu.modembed.model.textnotations.instructionset.ui.InstructionsetPositionCategory.DEFINTION) {
	...
	private void removeAnnotations() {
		removeAnnotations(hu.modembed.model.textnotations.instructionset.ui.InstructionsetOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(hu.modembed.model.textnotations.instructionset.ui.InstructionsetOccurrence.DECLARATION_ANNOTATION_ID);
	}
