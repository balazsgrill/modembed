/*
 * Library for PIC16LF1507
 * 
 * Architecture: 16Exxx
 */
library microchip.PIC16LF1507{
 ...
 ************************/
	 ...
	 */
		/* External Clock, High Power Mode */
		const uint8 CONFIG1_FOSC_ECH = 0x3*1;
		/* External Clock, Medium Power Mode */
		const uint8 CONFIG1_FOSC_ECM = 0x2*1;
		/* External Clock, Low Power Mode */
		const uint8 CONFIG1_FOSC_ECL = 0x1*1;
		/* Internal Oscillator, I/O function on OSC1 */
		const uint8 CONFIG1_FOSC_INTOSC = 0x0*1;
	/*
	 * Field: WDTE
	 ...
	 */
		...
		const uint8 CONFIG2_LVP_ON = 0x1*32;
		/* High-voltage on MCLR/VPP must be used for programming */
		const uint8 CONFIG2_LVP_OFF = 0x0*32;
